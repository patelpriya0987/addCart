    ...
    products.forEach((item) => {
        viewCart.innerHTML += `
                <div class="card m-2" style="width: 18rem;">
                    <img src="${item.thumbnail}" class="card-img-top" alt="img">
                    <div class="card-body">
                      <h5 class="card-title">${item.title}</h5>
                      <h4>price : $ ${item.price}</h4>
                      <p class="card-text">${item.description}</p>
                      <a href="#" class="btn btn-primary" onClick="return addToCart(${item.id})" >Add to cart</a>
                    </div>
                  </div>
                </div>
